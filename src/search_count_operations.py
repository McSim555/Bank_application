...
def process_bank_search(data_operations: list[dict], search_element: str) -> list[dict]:
    ...
    pattern = re.compile(search_element)

    filtered_operations = [
        operation
        for operation in data_operations
        if "description" in operation and pattern.fullmatch(operation["description"])
    ]
...
def process_bank_operations(data_operations:list[dict], operations_categories:list)->dict:
    """Функцию возвращает словарь, в котором ключи — это названия категорий, а значения — это количество операций в каждой категории"""
